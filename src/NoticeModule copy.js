...
export default function NoticeModule() {
  ...
  const fetchNotices = async () => {
    const { data } = await supabase.from("notice").select("*").order("id", { ascending: false });
    setNotices(data || []);
  };
  ...
  const createNotice = async () => {
    ...
    await supabase.from("notice").insert([{ title, message, date }]);
    fetchNotices();
  ...
  return (
    <div className="page" style={{ maxWidth: "800px", margin: "0 auto" }}>
      ...
      {/* Create Notice Form */}
      <section style={{ padding: "24px", background: "#f8fafc", borderRadius: "16px", marginBottom: "32px" }}>
        <h2>Create Notice</h2>
